...
extension UserDefaults.Name {
    ...
    static let FILTER_CONFIG : UserDefaults.Name = "filter_config"
...
extension UserDefaults {
    ...
    func saveObject(customObject object: NSCoding, forName name: UserDefaults.Name){ //2
        let encodedObject = NSKeyedArchiver.archivedData(withRootObject: object)
        self.set(encodedObject, forKey: name.rawValue)
        self.synchronize()
    }
    
    func getObject( key: UserDefaults.Name) -> NSObject{ //2
        
        let decodedObject = self.object(forKey: key.rawValue) as? NSData
        if(decodedObject == nil){
            return NSObject()
        }
        guard let object =  NSKeyedUnarchiver.unarchiveObject(with: decodedObject! as Data) else{return NSObject()}
        return (object as! NSObject)
    }
    
    func saveCustomObject(customObject object: NSCoding, forName : String){
        let encodedObject = NSKeyedArchiver.archivedData(withRootObject: object)
        self.set(encodedObject, forKey: forName)
        self.synchronize()
    }
